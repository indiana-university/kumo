...
actually builds from -- are generated on every build by [`scripts/sync_wiki.py`](scripts/sync_wiki.py),
...
`scripts/sync_wiki.py` or the files below.

What's actually committed here, and what each does:

| Path | Purpose |
| --- | --- |
| [`scripts/sync_wiki.py`](scripts/sync_wiki.py) | Generates `docs/` + `mkdocs.yml` from the wiki. The only place nav structure and wiki-link rewriting logic live. |
...
```powershell
python scripts/sync_wiki.py   # regenerate docs/ + mkdocs.yml from the current wiki
